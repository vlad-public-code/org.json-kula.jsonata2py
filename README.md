...
in memory, and handed back as a ready-to-call object. Evaluation is **~26x faster
than the pure-Python reference interpreter**, and faster than both Rust-backed
alternatives on the common case: querying Python objects you already hold
([see the benchmarks](#performance)).
...
- [**Performance**](#performance) — [vs. other PyPI implementations](#measured-against-the-other-pypi-implementations), [why it wins](#why-a-pure-python-library-beats-two-rust-ones-here), [break-even](#when-compilation-pays-for-itself)
